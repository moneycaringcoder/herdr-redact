...
//! memory, using the shipped built-in rules and no socket I/O or persistent
//! state. Run it with `cargo bench --bench scan_cost`. Cargo's bench profile
//! inherits the shipped `[profile.release]`, including `opt-level = "z"` and
//! LTO, so these are the scan costs the shipped binary actually gets.
//!
